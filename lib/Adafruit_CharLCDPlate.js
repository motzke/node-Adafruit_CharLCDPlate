...
function init(){
    ...
    _sendBytes(MCP23017_IOCON_BANK1, 0);

    _sendBytes(0, [ 0x3F, // IODIRA R+G LEDs=outputs, buttons=inputs
    ...
    _sendBytes(MCP23017_IOCON_BANK0, 0xA0);
...
function write(value, char_mode){
    ...
    if(DDRB & 0x10){
        ...
        _sendBytes(MCP23017_GPIOB, lo);
        while(true){
            _sendByte(hi);
            bits = _readByte();
            _sendBytes(MCP23017_GPIOB, [lo, hi, lo]);
            if ((bits & 0x2) === 0 ){
                break;
            }
        }
        PORTB = lo;
        DDRB &= 0xEF;
        _sendBytes(MCP23017_IODIRB, DDRB);
    }
    bitmask = PORTB & 0x01;
    if (char_mode){
        bitmask |= 0x80;
    ...
    if(typeof value === "string" ){
        ...
        for(var k=0; k<value.length; k++){
            ...
            if(data.length >=32 || k == last){
                ...
                _sendBytes(MCP23017_GPIOB, data);
                PORTB = data[-1];
                data = [];
            }
        }
...
function backlight(color){
    ...
    _sendBytes(MCP23017_GPIOA, PORTA);
    _sendBytes(MCP23017_GPIOB, PORTB);
}
